document.addEventListener('DOMContentLoaded', () => {
    const canvas = document.getElementById('foldingchairCanvas');
    const ctx = canvas.getContext('2d');
    const loadingOverlay = document.getElementById('loadingOverlay');
    const loadingProgress = document.getElementById('loadingProgress');
    const scrollIndicator = document.getElementById('scrollIndicator');
    
    // Total frames in the folding chair sequence
    const frameCount = 61;
    const currentFrame = index => {
        // Generates path like "folding chair Sequence/untitled.20.1.jpg"
        return `folding chair Sequence/untitled.20.${index}.jpg`;
    };
    ...
    const render = (image) => {
        ...
        let ratio  = Math.min(hRatio, vRatio); // Use min for object-fit: contain
        if (vpWidth <= 768) {
            ratio = ratio * 1.5;
        }
    ...
    // Stride loading - for 61 frames we load everything (stride = 1)
    const stride = 1;
    ...
    const textDesc = document.getElementById('foldingchairDesc');
